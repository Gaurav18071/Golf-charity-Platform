        ...
        <div className="mt-16 grid gap-6 lg:grid-cols-3 lg:items-end">
          ...
          {podium.map((item) => {
            ...
            return (
              <article
                key={item.position}
                className={`surface flex flex-col items-center rounded-2xl p-6 sm:p-8 text-center transition-all duration-300 hover:-translate-y-2 hover:shadow-xl ${orderClass} ${
                  item.featured
                    ? "border-2 border-amber-400 lg:-translate-y-6 shadow-md"
                    : ""
                }`}
              >
                <div
                ...
                <span className="mt-5 rounded-full bg-slate-100 px-3.5 py-1 text-xs sm:text-sm font-semibold text-slate-700">
                  {item.position}
                </span>
